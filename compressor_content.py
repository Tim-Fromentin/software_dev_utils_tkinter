...
def compressor_content(compressor_frame_widget):
    compressor_label = Label(compressor_frame_widget, text="Compressor", fg="black", bg="white", font=("Lato", 25), justify=RIGHT)
    compressor_label.pack()
    
    # Créer une étiquette pour l'image
    image_label = tk.Label(compressor_frame_widget, width=50, height=50)
    image_label.pack()
    ...
    b = Button(compressor_frame_widget, text="Process", command=lambda: process_action(choice_del_data.get()))
    b.pack()